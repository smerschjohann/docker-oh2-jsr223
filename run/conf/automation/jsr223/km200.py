...
km200_gateway_host = "HOST"
km200_gateway_password = "GW_PW"
km200_private_password = "PRIV_KEY"
...
needed_data = {
    ...
    "/system/heatSources/hs1/nominalFuelConsumption": {"type": "number", "name": "Heizung_NominalFuelConsumption"},
...
class Buderus(object):
    ...
    def get_data(self, path):
        ...
        rawtext = resp.read()
        text = self.__decryption(rawtext)
...
class BuderusUpdater(Rule):
    def __init__(self):
        self.__eventTrigger = [TimerTrigger("30 * * * * ?")]
    ...
    def execute(self, event):
        try:
            for path, target in needed_data.iteritems():
                ...
                item = ir.getItem(target["name"])
                if str(item.state) != status:
                    oh.postUpdate(target["name"], str(status))
        except Exception, ex:
            print "buderus error", ex

ohInit(automationManager, [BuderusUpdater()])
